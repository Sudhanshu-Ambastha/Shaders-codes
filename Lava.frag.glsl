/* ============================================================================
   Blackbody Lava
   Based on fizzer's Flowing Lava: "https://www.shadertoy.com/view/4djSzR"
   Adapted by ~bj (2014) from line 11 to line 67: "https://www.shadertoy.com/view/MdBSRW"
   modified via Sudhanshu Ambastha in 16/08/2026: "https://github.com/Sudhanshu-Ambastha/Shaders-codes/blob/main/Lava.frag.glsl" as per below
   Modifications from line 42 to 64:
   - Removed iChannel0 texture dependency by integrating procedural FBM noise
   - Refactored the lava() function for a self-contained, texture-independent render
  ============================================================================ */
...
// Modified: Removed iChannel0 dependency and replaced with procedural FBM noise gradients
vec3 lava(vec3 norm, float lavaHeight, vec3 p, float f0, vec2 coord, float time, float moveSpeed, vec3 diffuse, vec3 ro)
{
    ...
    float tex = 1.1 - fbm(uv * 4.0 + vec2(time * 0.1, 0.0));
    ...
    float temp = ((hot * 2.0 + 2.0) * tex - cold) * (tex + 0.3);
    temp = mix(glow * 1.0, smoothstep(0.0, 1.2, temp) * 1.6, mask) + embers * 4.0;
    
    vec3 lavaColor = blackbody(temp) * vec3(1.8, 0.7, 0.3);
    
    return diffuse * (1.0 - mask) + lavaColor + haze * vec3(0.5, 0.1, 0.05);
}
...
#define MOTIONBLUR_EMBERS    0
#define ADD_HEAT_GLOW        0
